...
|---|---|---|---|
| 1 | Клиент проходит Wizard и делает успешный тест-звонок без инженера. | IN PROGRESS | Wizard draft/submit API and 7-step UI flow implemented; test-call API added (`POST /api/calls/test-call`). Build/smoke автоматизированы через CI workflow `.github/workflows/ci.yml`; функциональный end-to-end сценарий P0 остаётся в работе. |
...
| 7 | RBAC, audit log и изоляция данных подтверждены тестами. | IN PROGRESS | Backend API tests cover no token, RBAC forbidden role, and tenant/org mismatch with TZ-format error envelope (`npm run test -w backend` in CI workflow `.github/workflows/ci.yml`); audit log remains pending. |
